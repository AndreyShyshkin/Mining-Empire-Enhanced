...
class CraftingUI {
	constructor(craftingSystem) {
		...
		// Check if updateNearbyCraftingStations exists, if not, use our own
		if (
			typeof this.craftingSystem.updateNearbyCraftingStations !== 'function'
		) {
			console.log(
				'CraftingUI: updateNearbyCraftingStations not found, adding fallback method'
			)
			this.craftingSystem.updateNearbyCraftingStations = function () {
				if (typeof this.scanForCraftingStations === 'function') {
					this.scanForCraftingStations()
				} else {
					console.error(
						'CraftingSystem missing both updateNearbyCraftingStations and scanForCraftingStations methods'
					)
				}
			}
		}
	...
	createUI() {
		...
		// Create the CSS for the UI
		this.createCSS()
	}

	// Creates CSS for the crafting UI
	createCSS() {
		// Check if the style already exists
		if (!document.getElementById('crafting-ui-styles')) {
		...
		if (this.uiElement) {
			try {
				...
				if (
					...
					typeof this.craftingSystem.scanForCraftingStations === 'function'
				) {
					console.log('Falling back to scanForCraftingStations...')
					this.craftingSystem.scanForCraftingStations()
				} else {
					console.error('No crafting station scanning method available')
				}
			} catch (err) {
				console.error('Error in showUI when updating crafting stations:', err)
			}
	...
	toggleUI() {
		console.log('toggleUI called, current state:', this.isCraftingOpen)
	...
	toggleCrafting() {
		console.log('toggleCrafting called')
...
// Make sure we replace the existing CraftingUI
