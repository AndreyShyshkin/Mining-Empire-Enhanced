...
export class Player extends Entity {
			...
			this.SM.currentScene.TC.LoadedLayers.forEach(layer => {
				...
				if (blockIndex !== -1) {
					...
					if (block.Image === Images.chest) {
						// Get the block's grid coordinates
						const blockX = Math.floor(block.transform.Position.X / 100)
						const blockY = Math.floor(block.transform.Position.Y / 100)

						// Mark the chest as opened in the WorldManager
						WorldManager.getInstance().markChestOpened(blockX, blockY)
						...
						layer.forEach(entity => {
							if (
								...
								if (Collisions.AABBtoAABB(entity.GetCollider(), col)) {
									...
									if (entity.curHp <= 0) {
										// CRITICAL FIX: More accurate grid coordinate calculation
										const blockX = Math.floor(entity.transform.Position.X / 100)
										const blockY = Math.floor(entity.transform.Position.Y / 100)
										const blockKey = `${blockX},${blockY}`

										console.log(
											`Breaking block at ${blockKey}, position: ${entity.transform.Position.X}, ${entity.transform.Position.Y}`
										)

										// Use the worldManager to mark this block as broken
										WorldManager.getInstance().markBlockBroken(blockX, blockY)

										// CRITICAL FIX: Verify the block was marked as broken
										if (
											!WorldManager.getInstance().isBlockBroken(blockX, blockY)
										) {
											console.error(
												`FAILED to mark block as broken at ${blockKey}!`
											)
										}
	...
	CreateLadder() {
		...
		// Grid coordinates for world manager tracking
		...
		if (
			...
			if (!flag) {
				// Mark that we've placed a ladder here using grid coordinates
				WorldManager.getInstance().addLadder(gridX, gridY)
				console.log(
					`Created ladder at grid coordinates ${gridX}, ${gridY} (pixel: ${x}, ${y})`
				)
